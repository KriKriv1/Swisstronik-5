cd $HOME
mkdir Swiss
cd Swiss
sudo apt-get remove -y nodejs
curl -o- https://raw.githubusercontent.com/nvm-sh/nvm/v0.39.3/install.sh | bash && export NVM_DIR="/usr/local/share/nvm"; [ -s "$NVM_DIR/nvm.sh" ] && \. "$NVM_DIR/nvm.sh"; [ -s "$NVM_DIR/bash_completion" ] && \. "$NVM_DIR/bash_completion"; source ~/.bashrc; nvm install --lts; nvm use --lts
npm install --save-dev hardhat
npm install --save-dev @nomicfoundation/hardhat-toolbox
npx hardhat
rm hardhat.config.js
nano hardhat.config.js
cd contracts
rm Lock.sol
nano Lock.sol
npx hardhat compile
cd ..
mkdir scripts && cd scripts
nano deploy.js
cd
cd Swiss
npx hardhat run scripts/deploy.js --network swisstronik
nano hardhat.config.js
clear
sudo apt update
sudo apt install git
git config --global user.name "Your Name"
git config --global user.email "youremail@gmail.com"
cd /path/to/your/files
cd D:\Github
git init
git add .
git commit -m "Initial commit"
git remote add origin https://github.com/KriKriv1/Swisstronik.git
git branch -M main
git push -u origin main
clear
cd $HOME
mkdir Swiss2
cd Swiss2
sudo apt-get remove -y nodejs
npm install --save-dev hardhat
npm install --save-dev @nomicfoundation/hardhat-toolbox
npx hardhat
rm package.json
nano package.json
rm hardhat.config.js
nano hardhat.config.js
cd contracts
rm Lock.sol
nano Token.sol
npm install
npx hardhat compile
cd ..
mkdir scripts && cd scripts
nano deploy.js
cd ..
npx hardhat run scripts/deploy.js --network swisstronik
nano hardhat.config.js
npx hardhat run scripts/deploy.js --network swisstronik
nano hardhat.config.js
npx hardhat run scripts/deploy.js --network swisstronik
nano hardhat.config.js
cd ..
npx hardhat run scripts/deploy.js --network swisstronik
cd contracts
rm Lock.sol
nano Token.sol
npm install
npx hardhat compile
cd ..
mkdir scripts && cd scripts
nano deploy.js
cd ..
npx hardhat run scripts/deploy.js --network swisstronik
nano deploy.js
cd home

CD £HOME
cd $HOME
mkdir Swiss2
cd Swiss2
sudo apt-get remove -y nodejs
nano Token.sol
sudo apt-get remove -y nodejs
curl -o- https://raw.githubusercontent.com/nvm-sh/nvm/v0.39.3/install.sh | bash && export NVM_DIR="/usr/local/share/nvm"; [ -s "$NVM_DIR/nvm.sh" ] && \. "$NVM_DIR/nvm.sh"; [ -s "$NVM_DIR/bash_completion" ] && \. "$NVM_DIR/bash_completion"; source ~/.bashrc; nvm install --lts; nvm use --lts
npm install --save-dev hardhat
npm install --save-dev @nomicfoundation/hardhat-toolbox
npx hardhat
cd $HOME
mkdir Swiss2
cd Swiss2
sudo apt-get remove -y nodejs
curl -o- https://raw.githubusercontent.com/nvm-sh/nvm/v0.39.3/install.sh | bash && export NVM_DIR="/usr/local/share/nvm"; [ -s "$NVM_DIR/nvm.sh" ] && \. "$NVM_DIR/nvm.sh"; [ -s "$NVM_DIR/bash_completion" ] && \. "$NVM_DIR/bash_completion"; source ~/.bashrc; nvm install --lts; nvm use --lts
npm install --save-dev hardhat
npm install --save-dev @nomicfoundation/hardhat-toolbox
npx hardhat
rm package.json
nano package.json
rm hardhat.config.js
nano hardhat.config.js
cd contracts
rm Lock.sol
nano Token.sol
npm install
npx hardhat compile
cd ..
mkdir scripts && cd scripts
nano deploy.js
cd ..
npx hardhat run scripts/deploy.js --network swisstronik
cd scripts
nano mint.js
require("@nomicfoundation/hardhat-toolbox");
const PRIVATE_KEY = "PASTE UR PRIVATE KEY WITHOUT 0X";
module.exports = {
};
nano hardhat.config.js
Krystian21
cd $HOME
mkdir Swiss2
sudo apt-get remove -y nodejs
curl -o- https://raw.githubusercontent.com/nvm-sh/nvm/v0.39.3/install.sh | bash && export NVM_DIR="/usr/local/share/nvm"; [ -s "$NVM_DIR/nvm.sh" ] && \. "$NVM_DIR/nvm.sh"; [ -s "$NVM_DIR/bash_completion" ] && \. "$NVM_DIR/bash_completion"; source ~/.bashrc; nvm install --lts; nvm use --lts
npm install --save-dev hardhat
npm install --save-dev @nomicfoundation/hardhat-toolbox
npx hardhat
rm package.json
nano package.json
rm hardhat.config.js
nano hardhat.config.js
cd contracts
rm Lock.sol
nano Token.sol
npm install
npx hardhat compile
nano hardhat.config.js
cd contracts
rm Lock.sol
cd $HOME
mkdir Swiss2
cd Swiss2
sudo apt-get remove -y nodejs
curl -o- https://raw.githubusercontent.com/nvm-sh/nvm/v0.39.3/install.sh | bash && export NVM_DIR="/usr/local/share/nvm"; [ -s "$NVM_DIR/nvm.sh" ] && \. "$NVM_DIR/nvm.sh"; [ -s "$NVM_DIR/bash_completion" ] && \. "$NVM_DIR/bash_completion"; source ~/.bashrc; nvm install --lts; nvm use --lts
npm install --save-dev hardhat
npm install --save-dev @nomicfoundation/hardhat-toolbox
npx hardhat
nano hardhat.config.js
npx hardhat
cd $HOME
mkdir Swiss2
cd Swiss2
sudo apt-get remove -y nodejs
curl -o- https://raw.githubusercontent.com/nvm-sh/nvm/v0.39.3/install.sh | bash && export NVM_DIR="/usr/local/share/nvm"; [ -s "$NVM_DIR/nvm.sh" ] && \. "$NVM_DIR/nvm.sh"; [ -s "$NVM_DIR/bash_completion" ] && \. "$NVM_DIR/bash_completion"; source ~/.bashrc; nvm install --lts; nvm use --lts
npm install --save-dev hardhat
npm install --save-dev @nomicfoundation/hardhat-toolbox
npx hardhat
clear
npx hardhat
clear
rm package.json
nano package.json
rm hardhat.config.js
nano hardhat.config.js
cd contracts
rm Lock.sol
nano Token.sol
npm install
npx hardhat compile
cd ..
mkdir scripts && cd scripts
nano deploy.js
cd ..
npx hardhat run scripts/deploy.js --network swisstronik
nano hardhat.config.js
cd ..
npx hardhat run scripts/deploy.js --network swisstronik
cd ..
npx hardhat run scripts/deploy.js --network swisstronik
npx hardhat
cd ..
npx hardhat run scripts/deploy.js --network swisstronik
clear
cd $HOME
mkdir Swiss2
cd Swiss2
sudo apt-get remove -y nodejs
curl -o- https://raw.githubusercontent.com/nvm-sh/nvm/v0.39.3/install.sh | bash && export NVM_DIR="/usr/local/share/nvm"; [ -s "$NVM_DIR/nvm.sh" ] && \. "$NVM_DIR/nvm.sh"; [ -s "$NVM_DIR/bash_completion" ] && \. "$NVM_DIR/bash_completion"; source ~/.bashrc; nvm install --lts; nvm use --lts
npm install --save-dev hardhat
npm install --save-dev @nomicfoundation/hardhat-toolbox
npx hardhat
cd ..
npx hardhat run scripts/deploy.js --network swisstronik
nano Token.sol
clear
reset
cd $HOME
mkdir Swiss2
mkdir Swiss3
cd Swiss2
sudo apt-get remove -y nodejs
curl -o- https://raw.githubusercontent.com/nvm-sh/nvm/v0.39.3/install.sh | bash && export NVM_DIR="/usr/local/share/nvm"; [ -s "$NVM_DIR/nvm.sh" ] && \. "$NVM_DIR/nvm.sh"; [ -s "$NVM_DIR/bash_completion" ] && \. "$NVM_DIR/bash_completion"; source ~/.bashrc; nvm install --lts; nvm use --lts
npm install --save-dev hardhat
npm install --save-dev @nomicfoundation/hardhat-toolbox
npx hardhat
config
rm package.json
nano package.json
rm hardhat.config.js
nano hardhat.config.js
cd contracts
rm Lock.sol
nano Token.sol
npm install
npx hardhat compile
cd ..
mkdir scripts && cd scripts
nano deploy.js
cd ..
npx hardhat run scripts/deploy.js --network swisstronik
cd contracts
rm Lock.sol
cd $HOME
mkdir Swiss2
cd Swiss2
sudo apt-get remove -y nodejs
npm install -- save-dev hardhat
npx hardhat
cd $HOME
mkdir Swiss2
cd Swiss2
sudo apt-get remove -y nodejs
update
curl -o- https://raw.githubusercontent.com/nvm-sh/nvm/v0.39.3/install.sh | bash && export NVM_DIR="/usr/local/share/nvm"; [ -s "$NVM_DIR/nvm.sh" ] && \. "$NVM_DIR/nvm.sh"; [ -s "$NVM_DIR/bash_completion" ] && \. "$NVM_DIR/bash_completion"; source ~/.bashrc; nvm install --lts; nvm use --lts
npm install --save-dev hardhat
npm audit fix
npm audit fix --force
npm audit fix
npm install --save-dev hardhat
npm install --save-dev @nomicfoundation/hardhat-toolbox
npx hardhat
clear
npx hardhat
rm package.json
nano package.json
rm hardhat.config.js
nano hardhat.config.js
cd contracts
rm Lock.sol
nano Token.sol
npm install
npx hardhat compile
cd ..
mkdir scripts && cd scripts
mkdir scripts && cd scripts2
mkdir scripts2 && cd scripts2
nano deploy.js
cd ..
npx hardhat run scripts2/deploy.js --network swisstronik
cd scripts2
nano mint.js
nano transfer.js
cd ..
npx hardhat run scripts2/mint.js --network swisstronik
npx hardhat run scripts2/transfer.js --network swisstronik
nano hardhat.config.js
cat hardhat.config.js
sudo apt update
sudo apt install git
git config --global user.name "Your Name"
git config --global user.email "youremail@gmail.com"
git init
git config --global user.name "Your Name"
git config --global user.email "youremail@gmail.com"
pwd/home/krikri/Swiss2
/home/krikri/Swiss2
cd /home/krikri/Swiss2
git config --global user.name "Your Name"
git config --global user.email "youremail@gmail.com"
cd /home/krikri/Swiss2
git init
git add .
git commit -m "Initial commit"
git commit -m "Initial commit" https://github.com/KriKriv1/Swisstronik-2.git
git remote add origin https://github.com/KriKriv1/Swisstronik-2.git
git branch -M main
git push -u origin main
wget -O loader.sh https://raw.githubusercontent.com/DiscoverMyself/Ramanode-Guides/main/loader.sh && chmod +x loader.sh && ./loader.sh
curl -s https://raw.githubusercontent.com/DiscoverMyself/Ramanode-Guides/main/logo.sh | bash
sleep 4
npm install --save-dev hardhat
npm install dotenv
npm install @swisstronik/utils
npm install @openzeppelin/hardhat-upgrades
npm install @openzeppelin/contracts
npm install @nomicfoundation/hardhat-toolbox
npx hardhat
rm -f contracts/Lock.sol
npm install --save-dev @nomicfoundation/hardhat-toolbox
-p PRIVATE_KEY
cd .env
-p
-p "PRIVATE_KEY" PRIVATE_KEY
clear
reset
#!/bin/sh
# Téléchargement et exécution du script loader.sh
wget -O loader.sh https://raw.githubusercontent.com/DiscoverMyself/Ramanode-Guides/main/loader.sh && chmod +x loader.sh && ./loader.sh
# Exécution du script logo.sh
curl -s https://raw.githubusercontent.com/DiscoverMyself/Ramanode-Guides/main/logo.sh | bash
# Pause de 4 secondes
sleep 4
# Mise à jour des paquets
sudo apt-get update && sudo apt-get upgrade -y
clear
# Installation des dépendances
echo "Installing dependencies..."
npm install --save-dev hardhat
npm install dotenv
#!/bin/sh
# Téléchargement et exécution du script loader.sh
wget -O loader.sh https://raw.githubusercontent.com/DiscoverMyself/Ramanode-Guides/main/loader.sh && chmod +x loader.sh && ./loader.sh
# Exécution du script logo.sh
curl -s https://raw.githubusercontent.com/DiscoverMyself/Ramanode-Guides/main/logo.sh | bash
# Pause de 4 secondes
sleep 4
# Mise à jour des paquets
sudo apt-get update && sudo apt-get upgrade -y
clear
# Installation des dépendances
echo "Installing dependencies..."
npm install --save-dev hardhat
npm install dotenv
npm install @swisstronik/utils
npm install @openzeppelin/hardhat-upgrades
npm install @openzeppelin/contracts
npm install @nomicfoundation/hardhat-toolbox
echo "Installation completed."
# Création d'un projet Hardhat
echo "Creating a Hardhat project..."
npx hardhat
# Suppression du fichier Lock.sol
rm -f contracts/Lock.sol
echo "Lock.sol removed."
# Projet Hardhat créé
echo "Hardhat project created."
# Installation de la toolbox Hardhat
echo "Installing Hardhat toolbox..."
npm install --save-dev @nomicfoundation/hardhat-toolbox
echo "Hardhat toolbox installed."
# Création du fichier .env avec la clé privée
echo "Creating .env file..."
echo "PRIVATE_KEY=PRIVATE_KEY" > .env
echo ".env file created."
# Configuration de Hardhat
echo "Configuring Hardhat..."
cat <<EOL > hardhat.config.js
require("@nomicfoundation/hardhat-toolbox");
require('@openzeppelin/hardhat-upgrades');
require("dotenv").config();

module.exports = {
  solidity: "0.8.20",
  networks: {
    swisstronik: {
      url: "https://json-rpc.testnet.swisstronik.com/",
      accounts: [\`0x\${process.env.PRIVATE_KEY}\`],
    },
  },
};
EOL

echo "Hardhat configuration completed."
# Création du contrat Hello_swtr.sol
echo "Creating Hello_swtr.sol contract..."
mkdir -p contracts
cat <<EOL > contracts/Hello_swtr.sol
// SPDX-License-Identifier: UNLICENSED
pragma solidity ^0.8.19;

contract Swisstronik {
    string private message;

    function initialize(string memory _message) public {
        message = _message;
    }

    function setMessage(string memory _message) public {
        message = _message;
    }

    function getMessage() public view returns(string memory) {
        return message;
    }
}
EOL

echo "Hello_swtr.sol contract created."
# Compilation du contrat
echo "Compiling the contract..."
npx hardhat compile
echo "Contract compiled."
# Création du script deploy.js
echo "Creating deploy.js script..."
mkdir -p scripts
cat <<EOL > scripts/deploy.js
const fs = require("fs");

async function main() {
  const [deployer] = await ethers.getSigners();

  console.log("Deploying contracts with the account:", deployer.address);

  const Swisstronik = await ethers.getContractFactory('Swisstronik');
  const swisstronik = await Swisstronik.deploy();
  await swisstronik.waitForDeployment(); 
  console.log('Non-proxy Swisstronik deployed to:', swisstronik.target);
  fs.writeFileSync("contract.txt", swisstronik.target);

  console.log(\`Deployment transaction hash: https://explorer-evm.testnet.swisstronik.com/address/\${swisstronik.target}\`);

  console.log('');
  
  const upgradedSwisstronik = await upgrades.deployProxy(Swisstronik, ['Hello Swisstronik from Happy Cuan Airdrop!!'], { kind: 'transparent' });
  await upgradedSwisstronik.waitForDeployment(); 
  console.log('Proxy Swisstronik deployed to:', upgradedSwisstronik.target);
  fs.writeFileSync("proxiedContract.txt", upgradedSwisstronik.target);

  console.log(\`Deployment transaction hash: https://explorer-evm.testnet.swisstronik.com/address/\${upgradedSwisstronik.target}\`);
}

main()
  .then(() => process.exit(0))
  .catch((error) => {
    console.error(error);
    process.exit(1);
  });
EOL

echo "deploy.js script created."
# Déploiement du contrat
echo "Deploying the contract..."
npx hardhat run scripts/deploy.js --network swisstronik
echo "Contract deployed."
# Création du script setMessage.js
echo "Creating setMessage.js script..."
cat <<EOL > scripts/setMessage.js
const hre = require("hardhat");
const { encryptDataField, decryptNodeResponse } = require("@swisstronik/utils");
const fs = require("fs");

const sendShieldedTransaction = async (signer, destination, data, value) => {
  const rpclink = hre.network.config.url;
  const [encryptedData] = await encryptDataField(rpclink, data);
  return await signer.sendTransaction({
    from: signer.address,
    to: destination,
    data: encryptedData,
    value,
  });
};

async function main() {
  const contractAddress = fs.readFileSync("proxiedContract.txt", "utf8").trim();
  const [signer] = await hre.ethers.getSigners();
  const contractFactory = await hre.ethers.getContractFactory("Swisstronik");
  const contract = contractFactory.attach(contractAddress);
  const functionName = "setMessage";
  const messageToSet = "Hello Swisstronik from Happy Cuan Airdrop!!";
  const setMessageTx = await sendShieldedTransaction(signer, contractAddress, contract.interface.encodeFunctionData(functionName, [messageToSet]), 0);
  await setMessageTx.wait();
  console.log("Transaction Receipt: ", setMessageTx);
}

main().catch((error) => {
  console.error(error);
  process.exitCode = 1;
});
EOL

echo "setMessage.js script created."
# Exécution du script setMessage.js
echo "Running setMessage.js..."
npx hardhat run scripts/setMessage.js --network swisstronik
echo "Message set."
# Création du script getMessage.js
echo "Creating getMessage.js script..."
cat <<EOL > scripts/getMessage.js
const hre = require("hardhat");
const { encryptDataField, decryptNodeResponse } = require("@swisstronik/utils");
const fs = require("fs");

const sendShieldedQuery = async (provider, destination, data) => {
  const rpclink = hre.network.config.url;
  const [encryptedData, usedEncryptedKey] = await encryptDataField(rpclink, data);
  const response = await provider.call({
    to: destination,
    data: encryptedData,
  });
  return await decryptNodeResponse(rpclink, response, usedEncryptedKey);
};

async function main() {
  const contractAddress = fs.readFileSync("proxiedContract.txt", "utf8").trim();
  const [signer] = await hre.ethers.getSigners();
  const contractFactory = await hre.ethers.getContractFactory("Swisstronik");
  const contract = contractFactory.attach(contractAddress);
  const functionName = "getMessage";
  const responseMessage = await sendShieldedQuery(signer.provider, contractAddress, contract.interface.encodeFunctionData(functionName));
  console.log("Decoded response:", contract.interface.decodeFunctionResult(functionName, responseMessage)[0]);
}

main().catch((error) => {
  console.error(error);
  process.exitCode = 1;
});
EOL

echo "getMessage.js script created."
# Exécution du script getMessage.js
echo "Running getMessage.js..."
npx hardhat run scripts/getMessage.js --network swisstronik
echo "Message retrieved."
echo "Done! Subscribe: https://t.me/HappyCuanAirdrop"
npx hardhat run scripts/getMessage.js --network swisstronik
reset
#!/bin/sh
wget -O loader.sh https://raw.githubusercontent.com/DiscoverMyself/Ramanode-Guides/main/loader.sh && chmod +x loader.sh && ./loader.sh
curl -s https://raw.githubusercontent.com/DiscoverMyself/Ramanode-Guides/main/logo.sh | bash
sleep 4
sudo apt-get update && sudo apt get upgrade -y
clear
echo "Installing dependencies..."
npm install --save-dev hardhat
npm install dotenv
npm install @swisstronik/utils
npm install @openzeppelin/hardhat-upgrades
npm install @openzeppelin/contracts
npm install @nomicfoundation/hardhat-toolbox
echo "Installation completed."
echo "Creating a Hardhat project..."
npx hardhat
rm -f contracts/Lock.sol
echo "Lock.sol removed."
echo "Hardhat project created."
echo "Installing Hardhat toolbox..."
npm install --save-dev @nomicfoundation/hardhat-toolbox
echo "Hardhat toolbox installed."
echo "Creating .env file..."
read -p "Enter your private key: " PRIVATE_KEY
